...
unordered_map<int, pair<int, int>>
    to_grid[6]; //각 팀번호의 진행 루트 번호를 좌표로 변환
int route_length[6] = {
    0,
}; //각 팀의 루트길이 => 라운드 연산하는데 사용
int team_dir[6] = {
    0,
}; //각 팀의 진행 방향
int team_mems[6] = {
  ...
  for (int idx = 1; idx <= m; idx++) {
    for (int i = 0; i < route_length[idx]; i++) {
      if (teams[idx][i] == 1) {
        ...
        //빼보기
        if (teams[idx][added_dir] == 0 || teams[idx][added_dir] == 3) {
          //이쪽이 진행방향
          //역방향으로 팀 수만큼 카운트
          team_dir[idx] = 1;
          ...
          while (teams[idx][subbed_dir] != 1 && teams[idx][subbed_dir]) {
            //리더가 아닐때까지
            teams[idx][subbed_dir] =
                teams[idx][modular(subbed_dir, 1, route_length[idx])] + 1;
            subbed_dir = modular(subbed_dir, -1, route_length[idx]);
            //루트에 팀이꽉차 있을 경우
          }
        } else if (teams[idx][subbed_dir] == 0 || teams[idx][subbed_dir] == 3) {
          team_dir[idx] = -1;
          while (teams[idx][added_dir] != 1 && teams[idx][added_dir]) {
            //리더가 아닐때까지
            teams[idx][added_dir] =
                teams[idx][modular(added_dir, -1, route_length[idx])] + 1;
